...
class BlockManager:
    ...
    def accept_block(self, block):
        if self.signing_key == block[4]:
            ...
            print('# BLOCK: DISCARD generated by self')
            return False
        
        if not validate_block(block):
            # discard invalid block
            print('# BLOCK: DISCARD: invalid block')
            return False

        index = block[0]
        if index > self.next_index:
            # assume this case won't happen
            print('# BLOCK: DISCARD index assumption violation')
            return False
        ...
        if ((index == 0 and block[2] != bytes(32))
            ...
            print('# BLOCK: DISCARD previous block mismatch')
            return False
        ...
        if index == self.next_index - 1:
            if block[1] < self.block_list[index][1]:
                ...
                print('# BLOCK: DISCARD old block')
                return False
        
        self.store_verified_block(block)
        print(f'# BLOCK: ACCEPTED length: {self.next_index}')
